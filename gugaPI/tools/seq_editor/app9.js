...
var lineSensorState={visible:false,connected:false,busy:false,selected:'ir3',
...
function lineSensorRender(){
  ...
  document.querySelectorAll('[data-cal-step]').forEach(function(button){
    ...
    button.querySelector('em').textContent=done?'已完成':'等待';
  });
}
...
function lineSensorRenderCal(values){
  ...
  var capturing=values.step&&values.step!=='none';
  $('lineCalState').textContent=capturing?'正在采集 '+values.step+' '+(values.samples||'0/64'):
    lineSensorState.calActive?'标定进行中':'未开始';
  document.querySelectorAll('[data-cal-step]').forEach(function(button){
    var current=button.dataset.calStep===values.step;
    button.classList.toggle('capturing',current);
    if(current)button.querySelector('em').textContent=values.samples||'采集中';
  });
  var allReady=['white','black','center','left','right'].every(function(step){return lineSensorState.calReady[step]});
...
async function lineSensorPoll(){
  if(!lineSensorState.visible||!lineSensorState.connected||lineSensorState.busy)return;
  lineSensorState.busy=true;lineSensorRender();
  try{
  ...
  finally{lineSensorState.busy=false;lineSensorRender()}
}
async function lineSensorAction(command,success){
  ...
  finally{lineSensorState.busy=false;lineSensorRender();setTimeout(lineSensorPoll,40)}
}
function LineSensorPage_OnShow(){
  lineSensorState.visible=true;lineSensorRender();lineSensorPoll();
  clearInterval(lineSensorState.pollTimer);lineSensorState.pollTimer=setInterval(lineSensorPoll,900);
}
function LineSensorPage_OnHide(){lineSensorState.visible=false;clearInterval(lineSensorState.pollTimer);lineSensorState.pollTimer=null}
...
$('btnLineSensorRefresh').onclick=lineSensorPoll;
...
$('btnLineCalBegin').onclick=async function(){
  if(await lineSensorAction('irsensor calib begin','标定已开始')){lineSensorState.calActive=true;Object.keys(lineSensorState.calReady).forEach(function(key){lineSensorState.calReady[key]=false})}
};
$('btnLineCalCancel').onclick=async function(){if(await lineSensorAction('irsensor calib cancel','已取消本次标定'))lineSensorState.calActive=false};
$('btnLineCalCommit').onclick=async function(){
  if(await lineSensorAction('irsensor calib commit','标定结果已提交到 RAM，请验证后显式保存')){lineSensorState.calActive=false;lineSensorState.dirty=true}
};
...
function lineSensorSimCommand(cmd){
  ...
  if(cmd==='irsensor stats')return'irsensor stats bytes='+(lineSensorSim.seq*15)+' frames='+lineSensorSim.seq+' header_errors=0 crc_errors=0 uart_errors=0 dropped=0 period_ms=10 average_ms=10\r\n> ';
