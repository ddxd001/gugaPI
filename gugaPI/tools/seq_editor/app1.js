...
function parseRawInstruction(tokens,hasIndex){
  ...
  if(op===23&&p.length>=4){
    return{op:23,p1:Number(p[1]),p2:0,until:5,conditionValue:0,
      ons:rawTarget(p[2]),ont:rawTarget(p[3])};
  }
  if(op===24&&p.length>=5){
    return{op:24,p1:Number(p[1]),p2:Number(p[2]),until:5,
      conditionValue:0,ons:rawTarget(p[3]),ont:rawTarget(p[4])};
  }
  if(op===25&&p.length>=3){
...
function formatRawInstruction(x,includeIndex,index){
  ...
  if(x.op===23){
    return prefix+'ball_hold '+x.p1+' '+x.ons+' '+x.ont;
  }
  if(x.op===24){
    return prefix+'ball_move '+x.p1+' '+x.p2+' '+x.ons+' '+x.ont;
  }
  if(x.op===25){
...
function parseSimAdd(cmd){
  ...
  if(op==='ball_hold'){
    return{op:23,p1:Number(p[3]),p2:0,until:5,conditionValue:0,
      ons:rawTarget(p[4]),ont:rawTarget(p[5])};
  }
  if(op==='ball_move'){
    return{op:24,p1:Number(p[3]),p2:Number(p[4]),until:5,
      conditionValue:0,ons:rawTarget(p[5]),ont:rawTarget(p[6])};
  }
  if(op==='ball_disable'){
    return{op:25,p1:0,p2:0,until:5,conditionValue:0,
