...
$('btnConnect').onclick=async()=>{
  ...
  try{port=await navigator.serial.requestPort();await port.open({baudRate:115200});
    ...
    (async()=>{try{while(true){var r=await reader.read();if(r.done)break;if(r.value){logc('rx',r.value.replace(/\r/g,''));if(typeof onSerialData==='function')onSerialData(r.value.replace(/\r/g,''),'rx');rxBuf+=r.value;if(r.value.includes('END')||r.value.includes('> ')){if(rxResolve){rxResolve(rxBuf);rxResolve=null}}}}}catch(e){if(typeof onSerialData==='function')onSerialData('[串口读取中断] '+e.message+'\n','error')}})();
    await refreshSlots();
