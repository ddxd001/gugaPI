...
$('btnConnect').onclick=async()=>{
  if(port){try{await reader.cancel();await reader.releaseLock();await writer.releaseLock();await port.close()}catch(e){}port=null;writer=null;$('btnConnect').textContent='Connect';$('btnRefresh').disabled=true;$('statusText').textContent='';logc('tx','[DISC]');return}
  try{port=await navigator.serial.requestPort();await port.open({baudRate:115200});
    var dec=new TextDecoderStream();port.readable.pipeTo(dec.writable);reader=dec.readable.getReader();
    var enc=new TextEncoderStream();enc.readable.pipeTo(port.writable);writer=enc.writable.getWriter();
    $('btnConnect').textContent='Disconnect';$('btnRefresh').disabled=false;$('statusText').textContent='Connected';logc('tx','[CONN]');
    (async()=>{try{while(true){var r=await reader.read();if(r.done)break;if(r.value){logc('rx',r.value.replace(/\r/g,''));if(typeof onRxCb==='function')onRxCb(r.value.replace(/\r/g,''),'rx');rxBuf+=r.value;if(r.value.includes('END')||r.value.includes('> ')){if(rxResolve){rxResolve(rxBuf);rxResolve=null}}}}}catch(e){}})();
    await refreshSlots();
  }catch(e){logc('tx','[ERR]'+e.message)}};
$('btnSim').onclick=async()=>{
  if(simMode){simMode=false;$('btnSim').textContent='Simulate';$('btnRefresh').disabled=true;$('statusText').textContent='';logc('tx','[SIM OFF]');return}
  simMode=true;port={fake:true};writer={fake:true};$('btnSim').textContent='Stop Sim';$('btnConnect').textContent='Connect';$('btnRefresh').disabled=false;$('statusText').textContent='SIM MODE';logc('tx','[SIM ON]');await refreshSlots()};
async function send(cmd){if(!writer)return'';logc('tx','> '+cmd);if(typeof onRxCb==='function')onRxCb('> '+cmd+'\n','tx');if(simMode)return simResponse(cmd);rxBuf='';await writer.write(cmd+'\r\n');return new Promise(r=>{rxResolve=r;setTimeout(function(){if(rxResolve){rxResolve(rxBuf);rxResolve=null}},400)})}
function simResponse(cmd){var resp='';if(cmd.startsWith('seq dump')){var s=+cmd.split(' ')[2];var a=simSlots[s];if(a&&a.length){resp='SEQ '+s+' '+a.length+'\r\n';a.forEach(function(i){resp+=OP[i.op]+' '+i.p1+' '+i.p2+' '+COND[i.until]+' '+(i.ons===255?'next':i.ons)+' '+(i.ont===255?'abort':i.ont)+'\r\n'});resp+='END\r\n> '}else{resp='SEQ '+s+' 0\r\nEND\r\n> '}}else if(cmd.startsWith('seq list')){for(var i=0;i<8;i++){var a2=simSlots[i];resp+='seq '+i+' '+(a2&&a2.length?'ok':'empty')+' count='+(a2?a2.length:0)+'\r\n'}resp+='> '}else if(cmd==='run clear'){simInstrs=[];resp='run clear: ok\r\n> '}else if(cmd.startsWith('run add')){var p=cmd.split(/\s+/);simInstrs.push({op:Object.keys(OP).find(function(k){return OP[k]===p[2]})||0,p1:+p[3]||0,p2:+p[4]||0,until:COND.indexOf(p[5])>=0?COND.indexOf(p[5]):0,ons:p[6]==='next'?255:p[6]==='abort'?255:+p[6]||0,ont:p[7]==='next'?255:p[7]==='abort'?255:+p[7]||0});resp='run add: ok\r\n> '}else if(cmd.startsWith('seq save')){var s2=+cmd.split(' ')[2];simSlots[s2]=simInstrs.map(function(x){return Object.assign({},x)});resp='seq save: ok\r\n> '}else if(cmd.startsWith('seq run')){resp='seq run: ok\r\n> '}else if(cmd.startsWith('seq del')){var s3=+cmd.split(' ')[2];simSlots[s3]=null;resp='seq del: ok\r\n> '}else{resp='> '}logc('rx',resp.replace(/\r/g,''));if(typeof onRxCb==='function')onRxCb(resp.replace(/\r/g,''),'rx');return new Promise(function(r){setTimeout(function(){r(resp)},50)})}
