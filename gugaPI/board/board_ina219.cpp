...
void ConfigureI2cPinsWithPullUps(const drivers::I2cControllerConfig *bus)
{
    DL_I2C_disableController(bus->i2c);
    DL_GPIO_initPeripheralInputFunctionFeatures(bus->sda_iomux,
                                                bus->sda_iomux_func,
                                                DL_GPIO_INVERSION_DISABLE,
                                                DL_GPIO_RESISTOR_PULL_UP,
                                                DL_GPIO_HYSTERESIS_DISABLE,
                                                DL_GPIO_WAKEUP_DISABLE);
    DL_GPIO_initPeripheralInputFunctionFeatures(bus->scl_iomux,
                                                bus->scl_iomux_func,
                                                DL_GPIO_INVERSION_DISABLE,
                                                DL_GPIO_RESISTOR_PULL_UP,
                                                DL_GPIO_HYSTERESIS_DISABLE,
                                                DL_GPIO_WAKEUP_DISABLE);
    DL_GPIO_enableHiZ(bus->sda_iomux);
    DL_GPIO_enableHiZ(bus->scl_iomux);
...
static drivers::Ina219Config g_ina219Config = {
    0,
...
{
    g_ina219Config.bus = Board_I2cBusFind("ina219");
    if (g_ina219Config.bus == 0) {
        return drivers::DRIVER_ERROR_NOT_INITIALIZED;
    }
    ConfigureI2cPinsWithPullUps(g_ina219Config.bus);
