...
> **当前硬件限制（2026-07-28）**：无 MCU DRV8876 板无法实施
> U9-11/12 与 U9-15/16 的网络交换，当前只启用右侧 `MR` 单电机。
> `FEATURE_LOCAL_MOTOR_RIGHT_ONLY=1`，
> `FEATURE_ENABLE_DIFFERENTIAL_CHASSIS=0`。本节列出的双轮、航向、定距、
> 循迹和动作目标仍是最终系统目标，不代表当前单电机台架能够运行。

## 3. 历史基线（外置 MCU MotorDriver）

本节至后续早期阶段记录保留用于追溯，不能当作当前接线或控制后端说明。自 2026-07-28 起，当前基线已改为 MSPM0G3519 直驱右侧 MR DRV8876 + TIMG8 QEI 的单电机台架配置，详见第 12、13 节和 `LOCAL_MOTOR_MIGRATION.md`。
...
| Shell 串口迁移 | 已完成 | UART3，PA13/RX，PA14/TX，115200 8N1；LoRa 暂停用 |
...
- 当前版本 v13，payload 215 字节；兼容加载 v1-v12 历史布局。旧布局缺少的电机斜坡、循迹和航向参数会补入兼容默认值；旧版默认组合 `364/364/32` 加载时自动迁移为 `1456/1456/33050 um`，并标记 dirty，等待 `param save` 写回。
...
> 当前右侧 MR 单电机配置不具备差速底盘能力，本节整体阻塞；只能先执行
> `LOCAL_MOTOR_MIGRATION.md` 中的单通道台架项目。
...
| 2026-07-28 | 本地电机联调 | Shell 迁移到 PA13/RX、PA14/TX | UART3 115200 8N1；释放 PC10/PC11，LoRa 因引脚冲突暂停用 |
...
| 2026-07-28 | 电机后端迁移（原计划） | 外置 MCU MotorDriver 改为 MSPM0G3519 直驱双 DRV8876，加入双 QEI、10 ms 速度环、50 ms 调度保护和 300 ms 续租保护 | 软件构建通过，但要求交换 U9-11/12 与 U9-15/16，实物无法实施，双轮方案取消 |
| 2026-07-28 | MR 单电机台架 | 保留电机板原网络，只启用通道 2：PB13/PWM2、PB7/GPIO2、PA10/NSLEEP2、PC0/PC1/TIMG8 QEI | `FEATURE_LOCAL_MOTOR_RIGHT_ONLY=1`；左桥保持休眠，差速底盘及全部上层运动功能禁用，待台架验收 |

## 13. 下一步

当前代码保留阶段一~五的上层实现和阶段六的参数持久化，但当前硬件只能组成右侧 MR 单电机台架。旧 MotorDriver 上的双轮硬件验收结果不能继承，双轮差速、航向、定距、循迹、路口和比赛动作均处于硬件阻塞状态。接线与单电机首测步骤见 `LOCAL_MOTOR_MIGRATION.md`。

近期优先任务（按顺序）：

1. **原网络静态验收**：确认 U9-3～6 未使用；U9-11 为 `GPIO2`、U9-12 为 `GPIO1`、U9-15/16 为 `GMR_LA/LB`，没有进行网络交换。
2. **安全状态验收**：确认 PA10/PA11 复位时为低；单电机固件运行期间 PA11/NSLEEP1 始终低、PB14 不输出 PWM、PB9/GPIO1 为低，PB15/PB16 不得配置为输出。
3. **右编码器验收**：不接 12V，手转 CN6 RIGHT/MR 电机编码器，确认 PC0=`GMR_RB`、PC1=`GMR_RA` 的 TIMG8 QEI 计数连续且方向可校准。
4. **右电机低功率验收**：把 `speed_max_duty` 限制到 10%，使用限流电源，架空执行 30 RPM 正转、停车、反转和续租超时测试。
5. **右速度环回归**：只对 MR 通道逐步测试 30/60/80/150 RPM、换向、调度超时和停车；未确认电流和温升前不提高占空比。
6. **决定双轮硬件方案**：重新布板、增加转接/飞线或更换接口后，恢复第二路硬件 QEI，再重新启用 `FEATURE_ENABLE_DIFFERENTIAL_CHASSIS`。
7. **双轮后再恢复上层验收**：依次复验差速底盘、HOLD/TURN、定距、循迹、动作序列和比赛模式。

> 当前可推进的运动测试只有右侧 MR 单电机台架。不要执行
> `chassis vel`、`heading` 运动命令、`lf start`、自动路口或 ActionRunner
> 行驶/转弯动作。

硬件测试前需要确认：

- 当前烧录的是 `FEATURE_LOCAL_MOTOR_RIGHT_ONLY=1` 配置。
- 只连接 CN6 RIGHT/MR，左侧 ML 不接电机。
- U9-1 提供 5V 逻辑电，CN1 提供限流 12V 功率电，所有 GND 共地。
- `speed_max_duty` 已临时限制到 10%，机构架空。
- PA11/NSLEEP1 可保持低且 PB15/PB16 未被驱动。
- 急停或断电开关可随时操作。
