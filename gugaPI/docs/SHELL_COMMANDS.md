...
仍会按上述局部超时规则停车，可按实车最大角速度适当增大`timeout`后保存到FRAM。
...
`road align set`设置转弯前的编码器对齐距离和对齐/圆弧转弯基础速度上限。距离范围`0..300 mm`，
速度范围`1..300 RPM`；执行`param save`后分别以`road_align_distance_mm`和
`road_align_rpm`写入FRAM。距离为0时从循迹直接进入滚动圆弧转弯。进入目标航向前
最后20°时开始累计有效线证据；到达目标航向时若已经连续确认2帧，会在同一轮控制更新
中恢复循迹模式及进入路口前的循迹基础速度。证据不足时才继续按路口速度移动捕线。

距离大于0时，循迹控制直接把运动控制权交给恒速航向定距控制。路口基础速度取“配置的
连续处理RPM”和“进入路口时的循迹基础RPM”绝对值中的较小值，并保留原行驶方向。编码器
达到对齐距离后不制动、不蠕行、不停车，立即以同一基础速度进入IMU航向圆弧转向。转弯
末段提前验证新线路；连续2帧有效数据且航向到达目标后立即无零速交接回循迹。普通
`heading distance`和`heading turn`仍保留各自原有的停车、停稳确认和超时行为。

正常路口流程不会调用`Chassis_Stop()`；捕线超时、IMU/编码器/底盘通信异常、取消或用户
停止仍会进入安全停车。圆弧外轮保持原有“基础速度+差速修正”目标并受
`road_turn_outer_max_rpm`封顶，内轮随转向强度连续插值到
`-road_turn_inner_reverse_max_rpm`。默认满转向端点为外轮220 RPM、内轮-120 RPM；以
90 RPM路口基础速度运行时平均前进分量约为50 RPM，因此不会插入人为停车，但转弯半径
比原对称输出更小。

`road turn set`依次设置左转角、右转角、转前对齐距离mm、路口连续处理RPM和移动黑线重捕获超时ms。
范围分别为`1..180`、`-180..-1`、`0..300`、`1..300`、`1..5000`。参数立即生效但
其中转角和重捕获超时仍为运行期参数，复位后恢复`90/-90/800`；对齐距离和速度会同步
...
| `heading_turn_max_rpm` | 0..1000 | 60 | 原地转弯最大轮速；连续圆弧达到满转向强度的差速基准（RPM） |
| `road_turn_outer_max_rpm` | 1..1000 | 220 | 自动路口圆弧外轮正转目标上限（RPM） |
| `road_turn_inner_reverse_max_rpm` | 0..1000 | 120 | 自动路口圆弧内轮最大反转速度（RPM） |
