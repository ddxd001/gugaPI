...
- `ACT_OP_TRACK_COURSE = 26`：执行循迹赛道任务；任务0当前按编码器一圈距离精停。
...
- `ACT_OP_TRACK_COURSE=26`、受保护的内置H2任务0、中间六路A线诊断、
  编码器一圈立即停车，以及比赛OLED题目/任务和冻结计时页面。

当前仍属于台架联调版本。加速度前馈和H3～H5默认槽位模板尚未实施。
