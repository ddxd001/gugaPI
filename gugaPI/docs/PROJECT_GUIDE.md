...
- `log`：日志接口，目前默认关闭
- `debug_uart`：UART5 调试串口输出，默认 115200-8-N-1
- `fault`：统一错误码和严重错误处理
...
- 生成宏：`DEBUG_UART_INST`
- 服务接口：`services/debug_uart.h`
- 当前测试程序：`app` 中注册 `debug_uart_count` 任务，每 1000 ms 打印一个从 0 开始递增的数字

调试串口由 `FEATURE_ENABLE_DEBUG_UART` 控制，计数测试由 `FEATURE_ENABLE_UART_COUNTER_TEST` 控制。
