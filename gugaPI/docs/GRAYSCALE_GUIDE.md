...
唯一例外是道路分类器处于有界的`observing`窗口：即使当前瞬时几何已经变成
`straight/unknown/lost`，循迹也保持基础速度直行，让分类器根据累计证据发布事件；事件发布后由路口控制器接管或
安全停车，避免循迹提前一帧停止导致自动转弯无法启动。弱跟踪耗尽后，弱响应同样按无效帧处理，
不执行无限保持或盲目搜线。
...
dirty；检查参数无误后执行一次`param save`写入ConfigStore双副本。
