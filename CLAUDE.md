...
- **MotorDriver physical mapping**: left wheel = M2, right wheel = M1; both 1456 counts/rev on the output shaft (`13 PPR × x4 hardware QEI × 28:1`). M1 counts via TIMG9 QEI, M2 via TIMG8 QEI (hardware quadrature, not GPIO interrupts). The calibrated rolling radius is stored precisely as `wheel_radius_um=33050`.
