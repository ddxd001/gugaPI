...
任务接口和调度器实现保持不变：`App_GrayscaleUpdate()` 每 1 ms 推进一次 ADC
流水线。通道序列为 `0,1..6,7,1..6`，每完成一组中间六路便原子发布位置帧，典型
周期约 7 ms；最外侧 0、7 路各在 14 ms 内刷新一次。读取失败会放弃当前流水线并从
通道 0 重新开始。
...
`gray_track_mask=0x7E` 定义通道 1..6 为主循迹区，位置计算只使用该掩码内的通道；
最外侧 0、7 只保留为道路/路口证据。线段成员门限
...
持续变化 125 帧的疑似卡死。异常位写入 `anomaly`；通道异常、标定错误、采集失败或
数据过期都会禁止闭环并停车。
...
循迹参数重启后加载；`lf kp/kd/maxcorr/slew/losthold/losttimeout` 可立即修改并同步标记
ConfigStore dirty。
...
10 ms 控制任务只在 `sequence` 变化时消费新完整帧，避免对同一帧重复计算微分：
...
`confidence` 只作诊断，不单独决定速度或停车。正常跟踪始终使用命令指定的基础速度；
硬件/过期/通道异常立即停车，`lost/multiple/wide` 连续 6 个完整位置帧仍未恢复时停车。
中心位置死区为 `±50`；`lost_hold_ms` 和 `lost_stop_ms` 仅保留为旧配置兼容字段。
...
本车从 FRAM 读取并固化为源码默认值的现场标定样本（更换传感器或安装高度后仍应
重新标定）：

```text
white: 3253 3217 3189 3316 3151 3011 2802 3188
black:  1010  934  737 2010 1548 1347  753 1362
span:   2243 2283 2452 1306 1603 1664 2049 1826
```
