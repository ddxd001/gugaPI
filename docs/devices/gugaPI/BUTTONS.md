...
- 所有 feature profile 上电均进入比赛 ARMED。
- B1 与 B3 的消抖电平同时保持 1000 ms，在比赛模式与 `dev-running` 间切换。
- 两键确认同时按下时立即清除 B1/B3 待处理事件并执行软件全停；保持满 1 秒只负责确认
  模式切换。比赛运行中提前松开会停留在 ARMED，不恢复原序列。
- 成功触发或中途取消后，两键都松开才会重新布防，防止持续按住反复切换。
- 单独短按仍保持比赛功能：ARMED 下 B1/B3 选择槽位，B2 启动；RUNNING 下 B2 停止。
- FAULT 状态不响应模式切换。`dev-running` 只改变运行态，不能启用 feature profile
  已经编译关闭的功能。
