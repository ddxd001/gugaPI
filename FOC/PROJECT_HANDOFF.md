...
| 串口步进 | 25 RPM |
| 目标范围 | ±400 RPM |
| 比例项 | `speed_error / 2`，输出为电流 ADC counts |
| 积分格式 | Q6 |
| 抗积分饱和 | 已实现 |
...
| 最大目标速度 | ±50 RPM |
| 位置死区 | 4 counts，约 0.088° |

位置环生成速度目标，之后仍经过速度 PI 和 d/q 电流 PI。当前位置保持需要的转矩受约 ±250 mA 的 q 轴电流限制。
...
| 编码器超时 | 超过 50 ms 无有效更新，三相桥进入 Hi-Z |
| nFAULT | GPIO 下降沿中断立即关闭三相桥 |
| 软件停止 | `x` 后三相 INL 拉低，电机滑行 |

编码器超时设置为 50 ms，是为了容纳一次阻塞式 115200 baud 状态行输出。若后续改为 DMA/非阻塞 UART，可以重新缩短此时间。
...
3. 实际到达约 247.1°。
4. `-` 指令将目标恢复到 237.4°。
5. 最终实际位置约 237.3°、速度 0 RPM、`Iq_ref=-6 mA`、`stop=none`。

最终原始误差约 1 个编码器 count，即约 0.022°。这是一次空载静态测试结果，不代表所有负载和温度条件下的精度保证。
...
1. 发送 `-` 将停止状态目标从默认 +25 RPM 调到 0。
2. 发送 `s`，观察 500 ms 对齐。
3. 依次发送 `+`，测试 25、50、75、100 RPM；每级等待稳定后发送 `f`。
...
- UART 使用阻塞发送；长状态行期间主循环暂停读取编码器，因此编码器丢失阈值暂设为 50 ms。
