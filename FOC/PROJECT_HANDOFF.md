...
- ADC0 MEM1 ISR 会确认 ADC1 MEM0 已完成后才使用三相结果；不完整样本不会进入电流环。
- `j` 可查看双 ADC 配对缺失、结果/触发溢出、下溢和 ISR 重入计数。
- `i` 的平均值和诊断峰值从 5 kHz 降采样流中计算，64 点窗口仍为 12.8 ms；
  软件过流保护继续检查全部 20 kHz 样本。
...
PWM 零点同步装载后的回归中，25/75 RPM 状态采样分别为 29/78 RPM；位置
`+10°` 后误差约 0.4°，返回后误差 0.0°。累计超过 1068 万组三相 ADC 样本时，
配对缺失、结果溢出、触发溢出、下溢、异常 IRQ 和 ISR 重入均为 0；完整 FOC
ISR 峰值约 30.03 us，未超过 37.5 us 预算。仍需用示波器确认实际 PWM 边沿和
ADC 采样窗口。

电流诊断统计降至 5 kHz 后，非统计周期 ISR 最近值为 783～796 tick，较此前
常见的 856～866 tick 约减少 2 us；最坏分支累计峰值为 983 tick（30.72 us），
仍无预算超限。25/75 RPM 回归为 30/74 RPM，累计超过 1676 万组 ADC 样本时
所有同步错误计数仍为 0。
