...
当前固件按下面参数把编码器 `counts/s` 换算成输出轴 RPM：

```text
448 CPR * 50 减速比 = 22400 counts/output-shaft revolution
```

也就是说，输出轴每转 1 圈约等于 22400 个编码器计数。如果实测 1 圈计数不是这个值，需要修改换算常量。
...
`motor pid` 输出的是原始 Q4.4 参数。默认 `kp=16`，约等于 `1.0% duty / RPM`。默认 `SPEED_MAX_DUTY=30`，第一次闭环测试时最大占空比限制为 30%。
