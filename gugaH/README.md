...
MSPM0G3519 工程。它不链接 `gugaPI`，只保留 H2–H7 所需的固定状态机、
控制算法和设备协议。
...
滤波，弯道保留现有灵敏参数；物理弯道前后250 mm内连续插值，避免参数
切换造成轮速阶跃。两题也共用H5的缓启动、直弯速度切换、最终缓减速以及
`lap_distance + 50 mm`里程完成判据；H6只把滚球目标换成B2/B3设定位置。
入弯速度转换单独使用30 RPM/s减速度，并提前完成后保留180 mm稳球距离；
出弯加速和最终停车仍使用40 RPM/s。
H7完全复用H4的不循线直行、缓启动、B点计时和缓停车策略，但像H6一样
允许在READY状态用B2/B3设置钢球目标位置。
...
- B1 短按：循环 H2–H7、CAL ZERO、CAL H2 LOOP、CAL GRAY；长按达到判定时间后，松开按键才启动。
- H6/H7 READY 下，B2/B3 每次调整 1 mm，长按连续调整，范围 ±100 mm；
