...
| 参数 | 默认值 | 单位 |
| --- | ---: | --- |
| `ball_pid_kp` | 55 | mdeg/mm |
| `ball_pid_ki` | 0 | mdeg/(mm·s) |
...
1. 保持 Kd=18、Ki=20，从 Kp=40 开始。回位太慢时每次增加 5；持续大幅往返时降低。
...
4. `ball status` 中 `pid_mdeg=总量/P/I/D` 显示四项分量；当前台架参数关闭 I，由 `hold_mdeg` 五点插值承担静态补偿，避免在允许误差带内继续积分而产生低频摆动。
5. 调试阶段先不要保存；连续多次稳定且不越过 ±80 mm 后再执行 `config save`。

示例：

```text
config set ball_pid_kp 55
config set ball_pid_ki 0
config set ball_pid_kd 20
